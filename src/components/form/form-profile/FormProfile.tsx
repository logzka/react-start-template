...
const schema = yup
  .object({
    ...
    age: yup
      .number()
      .positive('Age must be a positive number above 18')
      .integer()
...
const FormProfile = ({ t }: { t?: (v: string) => ReactNode | string }) => {
  ...
  return (
    <FormProfileStyled className="form form--profile" onSubmit={handleSubmit(onSubmit)}>
      ...
      <FormItemStyled className="form--item">
        <Controller
          ...
          control={control}
          render={({ field }) => (
            <Select returnObject={false} items={genders} placeholder={t('form.gender') as string} required {...field} />
          )}
        />
